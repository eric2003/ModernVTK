...
// 回调函数
void ErrorCallbackFunction(vtkObject* caller, unsigned long eventId, void* clientData, void* callData) {
    std::cerr << "VTK 错误: " << static_cast<char*>(callData) << std::endl;
}

void WarningCallbackFunction(vtkObject* caller, unsigned long eventId, void* clientData, void* callData) {
    std::cerr << "VTK 警告: " << static_cast<char*>(callData) << std::endl;
}
...
int main(int, char*[])
{
    ...
    calc->SetFunction("dot(Normals, Vectors)");
    calc->SetResultArrayName("DotProduct");

    // 添加观察者
    vtkNew<vtkCallbackCommand> errorCallback;
    errorCallback->SetCallback(ErrorCallbackFunction);
    calc->AddObserver(vtkCommand::ErrorEvent, errorCallback);

    vtkNew<vtkCallbackCommand> warningCallback;
    warningCallback->SetCallback(WarningCallbackFunction);
    calc->AddObserver(vtkCommand::WarningEvent, warningCallback);
    ...
    if (!scalars) {
        ...
        scalars = manualScalars.GetPointer();
    }
